...
# CLI usage
def usage_cli_complex(argv = sys.argv[1:], center=0j, radius=3., c=0, seed='', worker=multiprocessing.cpu_count()):
    global args
    parser = argparse.ArgumentParser()
    parser.add_argument("--record", metavar="DIR", help="record frame in png")
    parser.add_argument("--size", type=float, default=2.5, help="render size (2.5)")
    parser.add_argument("--center", type=complex, default=center, help="plane center(%s)" % center)
    parser.add_argument("--radius", type=float, default=radius, help="plane radius (%s)" % radius)
...
class ComplexPlane:
    ...
    def compute_chunks(self, method, params):
        params = [self.window_size, self.offset, self.scale, self.args.sampling] + params + [self.length]
        if self.args.worker >= 2:
            # Divide image length by number of worker
            params[-1] /= self.args.worker
            # Append chunk position
            params = map(lambda x: params + [x], range(self.args.worker))
            # Compute
