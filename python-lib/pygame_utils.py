...
class WavGraph(ScreenPart):
    ...
    def render(self, buf):
        ...
        for y in xrange(0, self.window_size[1]):
            ...
            for point, offset, color in ((left, -10, 0xf10000), (right, +10, 0x00f100), (mono, 0, 0xf1)):
                pixels[int(self.x_range + offset + ((self.x_range - abs(offset)) / 2.) * point / (MAX_SHORT/2.))][y] = color
        self.pixels = pixels
