...
class JuliaSet(Window, ComplexPlane):
    ...
    def render(self, frame):
        ...
        nparray = self.compute_chunks(compute_julia_set, [self.max_iter, self.c])
        self.blit(self.color_vector(nparray))
        self.draw_axis()
        self.draw_function_msg()
        self.draw_cpoint()
        print "%04d: %.2f sec: ./julia_set.py --seed '%s' --center '%s' --radius %s" % (frame, time.time() - start_time, self.c, self.center, self.radius)
        ...
        for e in pygame.event.get():
            ...
            if e.type == MOUSEBUTTONDOWN:
                ...
                if e.key == K_RETURN:
                    ...
                    if   e.key == K_e: step = 3/4.0
                    elif e.key == K_a: step = 4/3.0
